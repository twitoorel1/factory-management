...
/* CRUD
CREATE - login, isLogin, logout
READ - findOneById, findOneByUsername, getAllUsers
UPDATE - 
DELETE - 
*/
...
async function login(userExist: IUser) {
	const currentDateTime = new Date();
	const query = `UPDATE ${DB_NAME} SET jwt_ac_token = ?, jwt_rf_token = ?, last_connected = ? WHERE id = ?`;

	try {
		const accessToken = createAccessToken(userExist.id.toString());
		const refreshToken = createRefreshToken(userExist.id.toString());
		await queryDatabase(query, [accessToken, refreshToken, currentDateTime, userExist.id]);
...
async function getAllUsers() {
	const query = `SELECT id, username, max_actions, num_of_actions FROM ${DB_NAME};`;
