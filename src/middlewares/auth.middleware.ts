...
export const authMiddleware = async (req: Request, res: Response, next: NextFunction) => {
	...
	try {
		...
		if (user.blockActions === 1) return res.status(401).json({ error: 'You Are Blocked Now Please Again Connected Later' });
...
export const refreshTokenMiddleware = async (req: Request, res: Response, next: NextFunction) => {
	...
	try {
		...
		const user = await User.findOneById(Number(decoded.userId));
